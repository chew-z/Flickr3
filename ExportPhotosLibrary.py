...
# Author:
#   #   https://github.com/tymmej/ExportPhotosLibrary
# Based on:
...
def getArgs(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description='Exports Photos Library to directory', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('-s', '--source', default="/Volumes/Elements/Pictures/Photos Library.photoslibrary",
                        help='source, path to Photos.app library')
    parser.add_argument('-d', '--destination', default="/Volumes/Elements/Pictures",
                        help='destination, path to external directory')
    parser.add_argument('-c', '--compare', default=False,
                        help='compare files', action="store_true")
    parser.add_argument('-n', '--dryrun', default=False,
                        help='do not copy files (still creates empty folders)', action="store_true")
    parser.add_argument('-m', '--masters', default=False,
                        help='export masters instead of edited', action="store_true")
    group1 = parser.add_mutually_exclusive_group()
    group1.add_argument('-l', '--links', default=False,
                        help='use symlinks', action="store_true")
    group1.add_argument('-i', '--hardlinks', default=False,
                        help='use hardlinks', action="store_true")
    group2 = parser.add_mutually_exclusive_group()
    group2.add_argument(
        '-p', '--progress', help="show progress bar", default=True, action="store_true")
...
if __name__ == '__main__':
    ...
    shutil.copyfile(os.path.join(
        libraryRoot, 'database/Library.apdb'), databasePathLibrary)
    shutil.copyfile(os.path.join(
        libraryRoot, 'database/ImageProxies.apdb'), databasePathEdited)
            ...
            for row in connection3.execute("select M.imagePath, V.fileName, V.adjustmentUUID from L.RKVersion as V inner join L.RKMaster as M on V.masterUuid=M.uuid where V.modelId = ?", versionId):
                ...
                if not args.masters:
                    if adjustmentUUID != "UNADJUSTEDNONRAW" and adjustmentUUID != "UNADJUSTED":
                        connectionEdited.execute(
                            "SELECT resourceUuid, filename FROM RKModelResource WHERE resourceTag=?", [adjustmentUUID])
                        uuid, fileName = connectionEdited.fetchone()
                        p1 = str(ord(uuid[0]))
                        p2 = str(ord(uuid[1]))
                        sourceImage = os.path.join(
                            libraryRoot, "resources/modelresources", p1, p2, uuid, fileName)
                destinationPath = os.path.join(destinationDirectory, fileName)
                if args.verbose:
                    print(("\t(" + str(progress) + "/" + str(images) +
                          ") From:\t" + sourceImage + "\tto:\t" + destinationPath))
                if not os.path.isfile(destinationPath):
                    ...
                    if not args.dryrun:
                        try:
                            if args.links:
                                os.symlink(sourceImage, os.path.join(
                                    destinationDirectory, os.path.basename(sourceImage)))
                            elif args.hardlinks:
                                os.link(sourceImage, os.path.join(
                                    destinationDirectory, os.path.basename(sourceImage)))
                            else:
                            ...
                            failed += 1
                            print((
                                "Failed to copy: %s. Skipping this element." % sourceImage))
                else:
                    if args.verbose:
                                ...
                                if not args.dryrun:
                                    ...
                                    try:
                                        if args.links:
                                            ...
                                            shutil.copy(
                                                sourceImage, destinationDirectory)
                                    except IOError:
                                        failed += 1
                                        print((
                                            "Failed to copy: %s. Skipping this element." % sourceImage))

    print(("\nImages:\t" + str(images) + "\tcopied:\t" +
          str(copied) + "\tfailed:\t" + str(failed)))
